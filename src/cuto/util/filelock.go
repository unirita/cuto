...
func NewFileLock(path string) (FileLock, error) {
	if !filepath.IsAbs(path) {
		return FileLock(""), errors.New("Invalid absolute path.")
	}
	return FileLock(path), nil
}

// ファイルを利用して、ロックを行います。
// 引数で指定したミリ秒まで待機します。0以下を指定した場合は、1度だけロックに挑戦します。
// 他プロセスのロックが指定時間内に解けなかった場合は、ErrBusy を返します。
func (fl FileLock) Lock(timeout_millisec int64) error {
	err := fl.tryLock()
	...
	if err == ErrBusy { // Locked by other process.
		if timeout_millisec > 0 {
			...
			for {
				...
				d := time.Since(st)
				if d.Nanoseconds() > (timeout_millisec * 1000000) {
					return ErrBusy
				}
			}
		}
...
func (fl FileLock) existOwnerProcess() (bool, error) {
	name := string(fl)
